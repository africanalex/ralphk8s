...
# 3. Fall back to /etc/ralph/prompt.md (shared default)
PROMPT_FILE="${RALPH_PROMPT_FILE:-$SCRIPT_DIR/prompt.md}"
if [[ ! -f "$PROMPT_FILE" ]]; then
  DEFAULT_PROMPT_FILE="/etc/ralph/prompt.md"
...
# Claude CLI options (override via env)
AI_MODEL="${AI_MODEL:-${CLAUDE_MODEL:-claude-sonnet-4-5-20250929}}"
AI_TIMEOUT="${AI_TIMEOUT:-${CLAUDE_TIMEOUT:-600}}"  # 10 minutes
CLAUDE_ARGS=(
...
main() {
  ...
  log_note "Ralph started${RUN_NAME:+ ($RUN_NAME)}. Max iterations: $MAX_ITERATIONS. Model: $AI_MODEL. Prompt: $PROMPT_FILE"
  ...
  while (( i <= MAX_ITERATIONS )); do
    ...
    OUTPUT="$(
      timeout "$AI_TIMEOUT" \
        claude "${CLAUDE_ARGS[@]}" < "$PROMPT_FILE" 2>&1 \
        | tee /dev/stderr
    )"
    CLAUDE_RC=$?
    set -e
    ...
    if (( CLAUDE_RC == 124 )); then
      echo ""
      echo "ERROR: Claude invocation timed out after ${AI_TIMEOUT}s"
      log_note "Claude timed out after ${AI_TIMEOUT}s. Exiting."
      exit 1
    fi
